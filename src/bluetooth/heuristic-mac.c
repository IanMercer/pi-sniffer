...
void apply_mac_address_heuristics (struct Device* device)
{
    ...
    if (string_starts_with(device->mac, "8C:DE:52"))
    {
        ...
        set_name(device, "ISSC Technologies Corp.", nt_manufacturer, "mac");
    }
    ...
    {
        // Samsung phone
        set_name(device, "Samsung phone", nt_manufacturer, "mac");
        device->category = CATEGORY_PHONE;
    }
    else if (string_starts_with(device->mac, "e0:55:3d"))
    {
        // OEM to Sony? HP?
        set_name(device, "Cisco Meraki", nt_manufacturer, "mac");
    ...
    {
        // OEM to Sony? HP?
        set_name(device, "Cloud Network Tech", nt_manufacturer, "mac");// (Samoa) Inc
        device->category = CATEGORY_FIXED;
    }
    else if (string_starts_with(device->mac, "cc:04:b4"))
    {
        set_name(device, "Select Comfort", nt_manufacturer, "mac");
        device->category = CATEGORY_FIXED;
    }
    ...
    {
        set_name(device, "Logitech", nt_manufacturer, "mac");
        device->category = CATEGORY_FIXED;
    }
    ...
    {
        set_name(device, "VXI Corp", nt_manufacturer, "mac");
        device->category = CATEGORY_HEADPHONES;
    }
    else if (string_starts_with(device->mac, "cc:70:ed"))
    {
        set_name(device, "Cisco Systems", nt_manufacturer, "mac");
        device->category = CATEGORY_FIXED;
    }
    else if (string_starts_with(device->mac, "6c:9a:c9"))
    {
        set_name(device, "Valentine Radar", nt_manufacturer, "mac");
        device->category = CATEGORY_CAR;
    }
    else if (string_starts_with(device->mac, "00:1E:91"))
    {
        set_name(device, "KIMIN Electronic", nt_manufacturer, "mac");  // AirPlay - Denon?
        device->category = CATEGORY_CAR;
    }
    else if (string_starts_with(device->mac, "04:EE:03"))
    {
        set_name(device, "TI Chipset 1", nt_manufacturer, "mac");  // Chipset
    }
    else if (string_starts_with(device->mac, "A4:DA:32"))
    {
        set_name(device, "TI Chipset 2", nt_manufacturer, "mac");  // Chipset
    }
    else if (string_starts_with(device->mac, "E4:34:93"))
    {
        set_name(device, "Huawei", nt_manufacturer, "mac");
